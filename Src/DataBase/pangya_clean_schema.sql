...
`item_id` int(11) NOT NULL COMMENT 'Item ID (unique per UID, not global)',
...
CREATE TABLE IF NOT EXISTS `pangya_selfdesign` (
  ...
  `UCC_COPY_COUNT` int(11) DEFAULT '1',
  `IN_DATE` datetime(3) DEFAULT CURRENT_TIMESTAMP(3)
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;
...
ALTER TABLE `pangya_warehouse` ADD PRIMARY KEY (`item_id`,`UID`), ADD KEY `IX_Warehouse_UID` (`UID`);
