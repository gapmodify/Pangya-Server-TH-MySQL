...
namespace PangyaAPI.PangyaClient
{
    public abstract partial class Player : IDisposeable
    {
        ...
        public Player(TcpClient tcp)
        {
            ...
            // สร้าง DbContext ตาม database engine
            _db = DbContextFactory.CreateX();
        }
        ...
        public void SendBytes(byte[] buffer)
        {
            if (Tcp.Connected && Connected)
            {
                try
                {
                    if (Server != null && Server.ShowLog && buffer != null && buffer.Length > 0)
                    {
                        var decrypted = buffer.ServerDecrypt(GetKey);
                        WriteConsole.WriteLine($"[PACKET_SEND]: PacketSize({decrypted.Length})", ConsoleColor.DarkCyan);
                        WriteConsole.Write($"{decrypted.HexDump()}", ConsoleColor.DarkCyan);
        ...
        public void RefreshData()
        {
            while (true)
            {
                ...
                try
                {
                    // Refresh data สำหรับ MSSQL เท่านั้น
                    // MySQL ไม่ต้อง refresh เพราะใช้ raw SQL queries
                    // Note: Since this project only supports MySQL (via DbContextFactory),
                    // this refresh logic is effectively disabled
                    if (!DbContextFactory.IsMySql())
                    {
                        // MSSQL-specific refresh logic would go here
                        // Currently not supported as only MySQL is available
                    }
                }
                catch
                {
