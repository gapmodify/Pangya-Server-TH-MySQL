...
namespace PangyaAPI.Auth
{
    public class AuthClient : IDisposeable
    {
        ...
        public AuthClient(TcpClient client, AuthPacket packet)
        {
            ...
            using (var _db = DbContextFactory.CreateX())
            {
                var serverExists = _db.Database.SqlQuery<int>(
                    "SELECT COUNT(*) FROM pangya_server WHERE ServerID = @p0", Data.UID).FirstOrDefault() > 0;

                if (serverExists)
                {
                    var query = $"UPDATE pangya_server SET Active = 1 WHERE ServerID = {Data.UID}";
        ...
        public bool Connect()
        {
            try
            {
                if (string.IsNullOrEmpty(Data.AuthServer_Ip))
                {
                    Console.WriteLine("[AUTH_CLIENT_ERROR]: AuthServer_Ip is null or empty");
                    return false;
                }

                if (Data.AuthServer_Port <= 0)
                {
                    Console.WriteLine("[AUTH_CLIENT_ERROR]: AuthServer_Port is invalid");
                    return false;
                }

                if (Tcp == null)
                {
                    Console.WriteLine("[AUTH_CLIENT_ERROR]: TcpClient is null, creating new instance");
                    Tcp = new TcpClient();
                }
            ...
            {
                Console.WriteLine($"[AUTH_CLIENT_ERROR]: Failed to connect to AuthServer: {ex.Message}");
                if (ex.InnerException != null)
                {
                    Console.WriteLine($"[AUTH_CLIENT_ERROR]: Inner exception: {ex.InnerException.Message}");
                }
                return false;
            }
        }
