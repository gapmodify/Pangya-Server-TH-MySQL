...
namespace PangyaFileCore.Collections
{
    public class ItemCollection : Dictionary<uint, Item> 
    {
        ...
        public void DumpToCsv(string filePath)
        {
            ...
            // If a relative path is provided, save next to the executable for easy discovery.
            ...
            sb.AppendLine("TypeID,Name,Enabled,PriceType,MoneyFlag,ItemPrice");

            foreach (var kv in this.OrderBy(k => k.Key))
            {
                var it = kv.Value;
                var name = (it.Base.Name ?? string.Empty).Replace('"', '\'');

                sb.Append(kv.Key);
                sb.Append(",\"");
                sb.Append(name);
                sb.Append("\",");
                sb.Append(it.Base.Enabled);
                sb.Append(',');
                sb.Append(it.Base.PriceType);
                sb.Append(',');
                sb.Append(it.Base.MoneyFlag);
                sb.Append(',');
                sb.Append(it.Base.ItemPrice);
                sb.AppendLine();
            }
        ...
        public void DumpToCsvIfLoaded(string filePath)
        {
            if (this.Count == 0)
                return;
