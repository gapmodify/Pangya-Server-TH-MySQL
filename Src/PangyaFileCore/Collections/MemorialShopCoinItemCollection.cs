   ...
   public class MemorialShopCoinItemCollection : Dictionary<uint, MemorialCoinItem> 
    {
        ...
        public void Load()
        {
            ...
            using (var zip = ZipFile.OpenRead("data/pangya_th.iff"))
            {
                var FileZip = zip.Entries.FirstOrDefault(c => c.Name == "MemorialShopCoinItem.iff")
                           ?? zip.Entries.FirstOrDefault(c => c.Name == "MemorialShopCoinItem.sff");

                if (FileZip == null)
                {
                    throw new Exception("MemorialShopCoinItem.iff or MemorialShopCoinItem.sff not found in data/pangya_th.iff");
                }

                Buffer = new MemoryStream();
                using (var stream = FileZip.Open())
                {
