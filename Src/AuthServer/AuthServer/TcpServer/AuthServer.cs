...
namespace AuthServer
{
    public class AuthServer
    {
        ...
        public AuthServer()
        {
            try
            {
                string iniPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Auth.ini");
                
                // สร้างไฟล์ Auth.ini เฉพาะเมื่อยังไม่มีไฟล์เท่านั้น
                if (!File.Exists(iniPath))
                {
                    ...
                    File.WriteAllLines(iniPath, lines, System.Text.Encoding.ASCII);
                    WriteConsole.WriteLine($"[CONFIG_INIT]: Created Auth.ini at {iniPath}", ConsoleColor.Cyan);
                }
                else
                {
                    WriteConsole.WriteLine($"[CONFIG_INIT]: Auth.ini already exists, using existing file", ConsoleColor.Green);
                }
                
                var Ini = new IniFile("Auth.ini");
                WriteConsole.WriteLine($"[CONFIG_DEBUG]: Reading from file: {Ini.FilePath}", ConsoleColor.Yellow);

                // อ่านค่าจากไฟล์
                string configName = Ini.ReadString("Config", "Name");
                string configVersion = Ini.ReadString("Config", "Version");
                uint configUID = Ini.ReadUInt32("Config", "UID");
                uint configMaxPlayers = Ini.ReadUInt32("Config", "MaxPlayers");
                uint configPort = Ini.ReadUInt32("Config", "Port");
                string configIP = Ini.ReadString("Config", "IP");
                ...
                WriteConsole.WriteLine($"[CONFIG_LOADED]: Name={Data.Name}, Version={Data.Version}", ConsoleColor.Green);
                WriteConsole.WriteLine($"[CONFIG_LOADED]: UID={Data.UID}, Port={Data.Port}, IP={Data.IP}", ConsoleColor.Green);
                WriteConsole.WriteLine($"[CONFIG_LOADED]: MaxPlayers={Data.MaxPlayers}", ConsoleColor.Green);
                
                _server = new TcpListener(IPAddress.Parse(Data.IP), (int)Data.Port);
            }
            catch (Exception erro)
            {
                WriteConsole.WriteLine($"ERRO_START: {erro.Message}", ConsoleColor.Red);
                Console.ReadKey();
                Environment.Exit(0);
            }
        }
        ...
        public void Start()
        {
            try
            {
                // ✅ Skip InsertServer เพราะไม่จำเป็นสำหรับ AuthServer
        ...
        private void HandleClient(object obj)
        {
            ...
            try
            {
                ...
                client = new AuthClient(tcpClient, packet);

                Clients.Add(client);
                Console.Title = string.Format("Pangya Fresh Up! AuthServer - LOGIN: {0}, GAMES: {1}, MESSENGER: {2}", 
                    Clients.Model.Where(c => c.Data.Type == AuthClientTypeEnum.LoginServer).ToList().Count, 
                    Clients.Model.Where(c => c.Data.Type == AuthClientTypeEnum.GameServer).ToList().Count, 
                    Clients.Model.Where(c => c.Data.Type == AuthClientTypeEnum.MessengerServer).ToList().Count);
                ...
                while (tcpClient.Connected)
                {
                    ...
                    {
                        WriteConsole.WriteLine($"[CLIENT_PACKET_ERROR]: {ex.Message}", ConsoleColor.Red);
            ...
            {
                WriteConsole.WriteLine($"[CLIENT_HANDLE_ERROR]: {ex.Message}", ConsoleColor.Red);
                if (ex.InnerException != null)
                {
                    WriteConsole.WriteLine($"[CLIENT_HANDLE_ERROR_INNER]: {ex.InnerException.Message}", ConsoleColor.Red);
                }
        ...
        private void ClientConnected(AuthClient Server)
        {
            ...
            // ✅ Skip UpdateServer() - ไม่จำเป็นต้อง update database
            // UpdateServer();
        }
        #endregion
        ...
        public virtual void DisconnectClient(AuthClient Server)
        {
            ...
            // ✅ Skip UpdateClient() - ไม่จำเป็นต้อง update database
            // UpdateClient(Server);
        ...
        {
            // ✅ Skip database update - ไม่จำเป็นสำหรับ AuthServer
            // AuthServer ไม่ต้อง update pangya_server table
        }
