    ...
    public class TutorialCoreSystem
    {
        ...
        public static void PlayerTutorialMission(GPlayer player, Packet packet)
        {
            ...
            using (var db = DbContextFactory.Create())
            {
                try
                {
                    // Get tutorial data from database
                    string sql = "SELECT rookie, beginner FROM pangya_tutorial WHERE uid = @p0 LIMIT 1";
                    var tutorialData = db.Database.SqlQuery<dynamic>(sql, (int)player.GetUID).FirstOrDefault();
                        ...
                        case TutorialType.NewRookie:
                            {
                                ...
                                switch (data.MissionID)
                                {
                                    ...
                                    case 4:
                                        {
                                            data.MissionID = (uint)(tutorialData.rookie ?? 0);
                                        }
                                        break;
                                    case 8:
                                        {
                        ...
                        case TutorialType.Beginner:
                            {
                                ...
                                switch (data.MissionID)
                                {
                                    ...
                                    case 512:
                                        {
                                            data.MissionID = (uint)(tutorialData.beginner ?? 0);//missão 11
                                        }
                                        break;
                                    case 1024:
                                        {
                                            data.MissionID = (uint)(tutorialData.beginner ?? 0);//missão 12
                                        }
                                        break;
                                    case 2048:
                                        {
                                            data.MissionID = (uint)(tutorialData.beginner ?? 0);//missão 13
                                        }
                                        break;
                                    case 4096:
                                        {
                                            data.MissionID = (uint)(tutorialData.beginner ?? 0);//missão 14
                                        }
                                        break;
                                    case 8192:
                                        {
                                            data.MissionID = (uint)(tutorialData.beginner ?? 0);//missão 15
                                        }
                                        break;
                                }
                            }
