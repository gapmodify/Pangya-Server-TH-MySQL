...
namespace Game.Functions.Mail
{
    public class MailSender : List<MailItem>, IDisposable
    {
        ...
        {
            AddText("ยินดีด้วย! คุณเพิ่งทำภารกิจสอนเล่นสำเร็จ!");
            if (Option == 0)
            {
                switch (Value)
                {
        ...
        public void AddItemTutorialEvent(uint Option)
        {
            switch (Option)
            {
                case 1:
                    {
                        AddText("ยินดีด้วย! คุณจบการสอนเล่นระดับ Rookie แล้ว!");
                        //Caddie Papel
                ...
                case 2:
                    {
                        AddText("ยินดีด้วย! คุณจบการสอนเล่นระดับ Beginner แล้ว!");
                        //Item Power Potion
        ...
        public void Send(uint UID)
        {
            ...
            {
                // Insert mail using raw SQL instead of stored procedure
                string insertMailSql = @"
                    INSERT INTO pangya_mail 
                    (uid, sender_name, message, item_data, read_flag, sent_date)
                    VALUES (@p0, @p1, @p2, @p3, 0, NOW())";
                
                _db.Database.ExecuteSqlCommand(insertMailSql, 
                    (int)UID, 
                    Sender ?? "@GM", 
                    Message?.ToString() ?? "", 
                    GetSqlUpdateMailString());
            }
        }
