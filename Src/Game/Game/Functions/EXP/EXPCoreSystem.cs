...
using Game.Defines;
using Game.Client;
using Game.Functions.Mail;
using System;
using System.Collections.Generic;
using System.Linq;
using Connector.DataBase;

namespace Game.Functions.EXP
{
        ...
        protected void PlayerGetEXP(GPlayer player, uint Total)
        {
            ...
            try
            {
                if (IsUpdate)
                {
                    ...
                    {
                        string updateSql = "UPDATE pangya_user_statistics SET game_point = @p0, game_level = @p1 WHERE uid = @p2";
                        _db.Database.ExecuteSqlCommand(updateSql, (int)player.Exp, player.Level, (int)player.GetUID);
                    }
                    player.SendLevelUp();
                }
            }
