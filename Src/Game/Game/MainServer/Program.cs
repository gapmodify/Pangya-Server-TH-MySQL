...
namespace Game.MainServer
{
    class Program
    {
        ...
        {
            WriteConsole.WriteLine($"[VERSION_CHECK]: Server Version: '{_server.Data.GameVersion}' | Client Version: '{version}'", ConsoleColor.Cyan);
            
            if (_server.Data.GameVersion == version)
            {
                WriteConsole.WriteLine($"[VERSION_CHECK]: ✓ Version matched - Client accepted", ConsoleColor.Green);
            }
            else
            {
                WriteConsole.WriteLine($"[VERSION_CHECK]: ⚠ Version different but Client accepted (Accept All Mode)", ConsoleColor.Yellow);
            }
