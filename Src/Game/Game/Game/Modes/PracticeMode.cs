...
namespace Game.Game.Modes
{
    public class PracticeMode : GameBase
    {
        ...
        public override byte[] GameInformation()
        {
            ...
            response.WriteByte((byte)GAME_TYPE.HOLE_REPEAT); //is practice
        ...
        public override void PlayerLoadSuccess(GPlayer client)
        {
            ...
            if (GameStart != DateTime.MinValue)
            {
                ...
                if (fGameData.GameTime > 0)
                {
                    ...
                    WriteConsole.WriteLine($"[PRACTICE_LOAD_SUCCESS] ⏱️ Elapsed: {elapsedMs / 1000}s, Remaining: {remainingMinutes}:{remainingSeconds:D2}", ConsoleColor.Cyan);
                }
                else
                {
                    WriteConsole.WriteLine($"[PRACTICE_LOAD_SUCCESS] ♾️ No time limit mode (Elapsed: {elapsedMs / 1000}s)", ConsoleColor.Cyan);
                }
            }
            else
            {
                WriteConsole.WriteLine($"[PRACTICE_LOAD_SUCCESS] ⚠ GameStart not set - using elapsed=0", ConsoleColor.Yellow);
            }
