...
namespace Game.Game
{
    public abstract class GameBase
    {
        ...
        // ✅ NEW: PlayerHoleData as virtual method
        public virtual void PlayerHoleData(GPlayer player, Packet packet)
        {
        ...
        // ✅ NEW: Send to all EXCEPT sender (C++ game_broadcast equivalent)
        public void SendExcept(GPlayer sender, byte[] Data)
        {
            foreach (var p in Players)
            {
                if (p.ConnectionID != sender.ConnectionID)
                {
                    p.SendResponse(Data);
                }
            }
            ...
            for (int i = 1; i <= count; ++i)
            {
                ...
                switch ((TGameShift)action)
                {
                    ...
                    case TGameShift.SHIFT_NATURAL: //natural mode
                        ...
                        break;
                    case TGameShift.SHIFT_ARTIFACT: //artifact setting (disabled)
                        {
                            packet.ReadUInt32(out uint ArtifactValue);
                            // Artifact is disabled - ignore the value
                            WriteConsole.WriteLine($"[PlayerGameSetting]: Artifact setting received (value={ArtifactValue}) - ignoring (artifacts disabled)", ConsoleColor.Yellow);
                        }
                        break;
                    default:
        ...
        public void HandlePacket(TGAMEPACKET ID, GPlayer player, Packet packet)
        {
            switch (ID)
            {
                ...
                case TGAMEPACKET.PLAYER_QUIT_CHIPIN_MODE:
                    {
                        WriteConsole.WriteLine($"[QUIT_PRACTICE] 👋 Player {player.GetNickname} quitting practice/chip-in mode", ConsoleColor.Cyan);
                        PlayerLeavePractice();
                    }
                    break;
